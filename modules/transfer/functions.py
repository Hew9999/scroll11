...
from helpers.web3_helper import check_data_token, add_gas_price, add_gas_limit, sign_tx, check_status_tx


def transfer(web3, private_key: str, to_address: str, chain: str, _token_address: str, _amount: float, retry=0):
    try:
        ...
        if not _amount:
            if token_address == NATIVE_TOKEN_ADDRESS:
                ...
                value = token_contract.functions.balanceOf(web3.to_checksum_address(wallet)).call()
        else:
        ...
        if retry < MAX_RETRIES:
            ...
            sleeping(7, 8)
            transfer(web3, private_key, to_address, chain, _token_address, _amount, retry + 1)
        else:
            cprint(f'{STR_CANCEL} Max retries reached, STOP.', 'red')


def map_recipients(recipients, private_keys):
    if len(recipients) != len(private_keys):
        cprint('Recipients and private keys have different wallets amount.', 'red')
        raise Exception('Error in map_recipients function.')

    result_map = {}
    for index, recipient in enumerate(recipients):
        result_map[private_keys[index]['private_key']] = recipient
    return result_map
